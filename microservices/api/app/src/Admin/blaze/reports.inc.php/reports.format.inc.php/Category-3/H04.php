	...
	while($row1=mysqli_fetch_array($query1))
	{
		...
		while($row2=mysqli_fetch_array($query2))
		{
			...
			else
			{
				...
				while($nrw<$lmt&&($cls!="FY Instrumentation"))
				{
...
AND session='$session' AND canc<>1 AND pcan<>1 AND form_cat='$cat_s' ORDER BY ABS(sgpa) LIMIT 1")or die(mysqli_error($mysqli));
						while($row55=mysqli_fetch_array($query55))
						{
							$id=$row55['sysid'];
						if($mycat1=="PH")
...
pclass='$mycls'  AND pgender='$mygen' AND session='$session' AND canc<>1 AND pcan<>1 AND form_cat='$cat_s' ORDER BY ABS(sgpa) 
