...
$cat_s=$_POST['ses_cat'];
	...
	$query12=mysqli_query($mysqli,"select * from form_data where canc=1 AND ver=1 ORDER BY ABS(sgpa), pclass,pgender")or die(mysqli_error($mysqli));
...
AND session='$session' AND canc<>1 ORDER BY ABS(sgpa) LIMIT 1")or die(mysqli_error($mysqli));
						while($row55=mysqli_fetch_array($query55))
						{
							$id=$row55['sysid'];
						if($mycat1=="PH")
...
pclass='$mycls'  AND pgender='$mygen' AND session='$session' AND canc<>1 ORDER BY ABS(sgpa) 
...
SET stype='Not Alloted', sagn='Not Alloted', pcan=1 where sysid='$ido'")or die(mysqli_error($mysqli));
	...
	while($row1=mysqli_fetch_array($query1))
	{
		...
		while($row2=mysqli_fetch_array($query2))
		{
			...
			//alert($vss);
					$query5=mysqli_query($mysqli,"SELECT * FROM form_data WHERE ver=1 AND accd<>1 AND pclass='$cls'  AND pgender='$gender' AND session='$session' AND canc<>1 ORDER BY ABS(sgpa)")or die(mysqli_error($mysqli));
