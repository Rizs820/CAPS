	...
	while($row1=mysqli_fetch_array($query1))
	{
		...
		while($row2=mysqli_fetch_array($query2))
		{
			...
			else
			{
				...
				$query5=mysqli_query($mysqli,"SELECT * FROM form_data WHERE ver=1 AND accd<>1 AND pclass='$cls'  AND pgender='$gender' AND session='$session' AND form_cat='$cat_s' ORDER BY ABS(sgpa) ASC")or die(mysqli_error($mysqli));
				while($row5=mysqli_fetch_array($query5))
				{	
					$id=$row5['sysid'];
