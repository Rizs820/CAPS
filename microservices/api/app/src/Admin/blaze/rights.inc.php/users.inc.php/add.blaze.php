...
if($myRequestArray[0]=="Edit_User_Rights")
{
    $m_mode="EDIT RIGHTS (E)";
    $uid=$myRequestArray[1];
    $query = "SELECT user_id,access_list FROM rights_user WHERE uid = ? ORDER BY uid";
    ...
    else
    {
        ...
        $stmt->execute();
        $stmt->bind_result($user_id,$pageid);
        $stmt->fetch();
...
?>
    ...
    <div class="container-fluid">
            <div class="block-header">
                <h2>RIGHTS MANAGER (RM) / USER RIGHTS (UR) / <?php echo $m_mode;?></h2>
            </div>    
            <!-- Vertical Layout -->
                            ...
                            <form action="" method="POST" >
                                ...
                                <div class="form-group">
                                    <div class="form-line">
                                        ...
                                        <?php
                                            $query=mysqli_query($mysqli,"SELECT uid,user_name FROM members") or die(mysqli_query($mysqli));
                                            while($row=mysqli_fetch_array($query))
                                            {
                                                echo $user_id==$row['uid'] ? '<option value="'.$row['uid'].'" selected>'.$row['user_name'].'</option>' : '<option value="'.$row['uid'].'">'.$row['user_name'].'</option>';
                                            }
                                        ?>
                                        </select>
                                    </div>
                               ...
                               <label for="pageid">Select Page</label>
                                <div class="form-group">
                                    <div class="form-line">
                                        ...
                                        <?php
                                            $query=mysqli_query($mysqli,"SELECT token,name FROM access_list") or die(mysqli_query($mysqli));
                                            while($row=mysqli_fetch_array($query))
                                            {
...
if(isset($_POST['add_rights']))
{
    ...
    $p_user_id=mysqli_real_escape_string($mysqli,$p_user_id);
    $p_user_id=htmlentities($p_user_id);
    $p_user_id=htmlspecialchars($p_user_id);
    ...
    **/
    $stmt = $mysqli->prepare("INSERT INTO rights_user(user_id,access_list) VALUES (?, ?)");
    $stmt->bind_param('ss', $p_user_id, $p_pageid);
    if($stmt->execute())
...
if(isset($_POST['update_rights']))
{
    ...
    **/
    $stmt = $mysqli->prepare("UPDATE rights_user SET user_id = ?, access_list= ? WHERE uid = ?");
    $stmt->bind_param('sss', $p_user_id, $p_pageid, $uid);
    if($stmt->execute())
    {
        alert_sweet_success_wr("Rights Details Updated Successfully!!!","./?act=rights/users/modify");
