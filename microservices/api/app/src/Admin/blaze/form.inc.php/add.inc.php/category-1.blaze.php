...
if($myRequestArray[0]=="Edit_Form_C1")
{
...
?>
    ...
    <div class="container-fluid">
            <div class="block-header">
                <h2>FROM MANAGER (FM) / <?php echo $m_mode;?> / SY-LY STUDENTS (STL) / <?php echo $session_year;?></h2>
            </div>    
            <!-- Vertical Layout -->
                    ...
                    <div class="card">
                        ...
                        <div class="body">
                            <form action="" method="POST" name="user_profile">
                                <label for="page_name">PRN</label>
                                <div class="form-group">
                                    <div class="form-line">
                                        <input type="text" id="prn" name="prn" class="form-control" placeholder="Enter the Student PRN" value="<?php echo $prn;?>" required>
                                    </div>
                                </div>
                                ...
                                <div class="form-group">
                                    <div class="form-line">
                                        ...
                                        $query=mysqli_query($mysqli,"select * from department where hasres=1")or die(mysqli_error());
                                            ...
                                            {
                                                $d=$row['name'];
                                                $query1=mysqli_query($mysqli,"select * from cls WHERE stud_cat='Category-1'")or die(mysqli_error());
                                ...
                                <div class="form-group">
                                    <div class="form-line">
                                        <input type="text" id="sgpa" name="sgpa" class="form-control" placeholder="Enter the Latest Semester SGPA" value="<?php echo $sgpa;?>" required>
                                    </div>
                                </div>
                                <b>
                                <div class="row">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-5" align="center">
                                        If Fail / ATKT, Enter No. of subjects failed
                                    </div>
                                    <div class="col-md-2" align="center">
                                        CGPA
                                    </div>
                                ...
                                <?php
                                    ...
                                    ?>
                                        <div class="row">
                                            ...
                                            <div class="col-md-3" align="center">
                                                <select class="form-control show-tick" name="<?php echo "result".$i;?>">
                                                    <option value="">-- Please select --</option>
                                                    <?php 
                                                        echo ${"result".$i}=="Not Applicable"?"<option selected='yes'>Not Applicable</option>":'<option>Not Applicable</option>';
                                                        echo ${"result".$i}=="Pass"?"<option selected='yes'>Pass</option>":'<option>Pass</option>';
                                                        echo ${"result".$i}=="ATKT"?"<option selected='yes'>ATKT</option>":'<option>ATKT</option>';
                                                        echo ${"result".$i}=="Fail"?"<option selected='yes'>Fail</option>":'<option>Fail</option>';
                                                    ?>
                                                </select>
                                            </div>
                                            <div class="col-md-5" align="center">
                                                <select class="form-control show-tick" name="<?php echo "back".$i;?>">
                                                    <?php 
                                                        $j=0;
                                                        while($j<6)
                                                        {
                                                            echo ${"back".$i}==$j?"<option selected='yes'>".$j."</option>":'<option>'.$j.'</option>';
                                                            $j++;
                                                        }
                                                    ?>
                                                </select>
                                            ...
                                            <div class="col-md-2" align="center">
                                                <div class="form-group">
                                                    <div class="form-line">
                                                        <input type="text" class="form-control" placeholder="CGPA" name="<?php echo "sgpa".$i;?>" value="<?php echo ${"sgpa".$i};?>"/>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <?php
                                    ...
                                    {
                                ?>
                                <button type="submit" name="add_form_c1" value="add_from_c1" class="btn btn-block btn-lg btn-primary m-t-20 waves-effect">ADD NEW FORM</button>
                                <?php 
                                    }
                                    else
                                    {
                                ...
                                <button type="submit" name="update_form_c1" value="update_form_c1" class="btn btn-block btn-lg btn-primary m-t-20 waves-effect">UPDATE FORM DETAILS</button>
                                <?php 
                                    }
                                ?>
                            </form>
...
if(isset($_POST['add_form_c1']))
{
    ...
    **/
    $stmt = $mysqli->prepare("INSERT INTO form_data(prn,pname,pemail,pc_no,psc_no,pgender,sgpa,pclass,category,date,uid,ladd,padd,ph,def,jk,session,form_cat,s1_result,s1_back,s1_sgpa,s2_result,s2_back,s2_sgpa,s3_result,s3_back,s3_sgpa,s4_result,s4_back,s4_sgpa,s5_result,s5_back,s5_sgpa,sumf,f_version) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,  ?, ?, ?)");
    $stmt->bind_param('sssssssssssssssssssssssssssssssssss', $p_prn, $p_pname, $p_pemail, $p_pc_number, $p_psc_number, $p_pgender, $p_sgpa, $p_pbranch, $p_pcategory, $p_date, $p_suid, $p_ladd, $p_padd, $p_ph, $p_defence, $p_jk, $p_session, $p_form_cat, $result1, $back1, $sgpa1, $result2, $back2, $sgpa2, $result3, $back3, $sgpa3, $result4, $back4, $sgpa4, $result5, $back5, $sgpa5, $sm, $p_fversion);
    if($stmt->execute())
...
if(isset($_POST['update_form_c1']))
{
    ...
    **/
    $stmt = $mysqli->prepare("UPDATE form_data SET prn = ?,pname = ?,pemail = ?,pc_no = ?,psc_no = ?,pgender = ?,sgpa = ?,pclass = ?,category = ?,date = ?,ladd = ?,padd = ?,ph = ?,def = ?,jk = ?,s1_result = ?,s1_back = ?,s1_sgpa = ?,s2_result = ?,s2_back = ?,s2_sgpa = ?,s3_result = ?,s3_back = ?,s3_sgpa = ?,s4_result = ?,s4_back = ?,s4_sgpa = ?,s5_result = ?,s5_back = ?,s5_sgpa = ?,f_version = ? WHERE sysid = ?");
    $stmt->bind_param('ssssssssssssssssssssssssssssssss', $p_prn, $p_pname, $p_pemail, $p_pc_number, $p_psc_number, $p_pgender, $p_sgpa, $p_pbranch, $p_pcategory, $p_date, $p_ladd, $p_padd, $p_ph, $p_defence, $p_jk, $result1, $back1, $sgpa1, $result2, $back2, $sgpa2, $result3, $back3, $sgpa3, $result4, $back4, $sgpa4, $result5, $back5, $sgpa5, $p_fversion, $uid);
    if($stmt->execute())
    {
        alert_sweet_success_wr("Form Details Updated Successfully!!!","./?act=form/modify/category-1");
